...
// rb_tree : 红黑树
#ifndef STL_STUDY_RB_TREE_H
#define STL_STUDY_RB_TREE_H
...
namespace mystl
{
    ...
    //前置声明
    template<class T>struct rb_tree_node_base; //rb_tree 的节点基类
    template<class T>struct rb_tree_node; //rb_tree 的节点类
    template<class T>struct rb_tree_iterator; //rb_tree 的迭代器类
    template<class T>struct rb_tree_const_iterator; //rb_tree 的常量迭代器类

    //rb tree value traits //rb_tree 的节点值 traits
    template<class T,bool>
    struct rb_tree_value_traits_imp
    {
        typedef T key_type; //键值类型
        typedef T mapped_type; //map 的节点值类型
        typedef T value_type; //节点值类型
        template<class Ty>
                static const key_type& get_key(const Ty& value) //获取键值
        {
            return value;
        }

        template<class Ty>
                static const value_type& get_value(const Ty& value)//获取节点值
        {
            return value;
        }
    };
    ...
    template<class T>
    struct rb_tree_value_traits_imp<T,true> //偏特化，当节点值为 pair 类型时
    {
        typedef typename std::remove_cv<typename T::first_type>::type key_type; //键值类型
        typedef typename T::second_type mapped_type; //map 的节点值类型
        typedef T value_type; //节点值类型
    ...
    template<class T>
    //struct rb_tree_value_traits : public rb_tree_value_traits_imp<T,mystl::is_pair<T>::value>{};
    struct rb_tree_value_traits
    {
        static constexpr bool is_map = mystl::is_pair<T>::value;//判断是否为 map

        typedef rb_tree_value_traits_imp<T,is_map> value_traits;
        typedef typename value_traits::key_type key_type;//键值类型
        typedef typename value_traits::mapped_type mapped_type;//map 的节点值类型
        typedef typename value_traits::value_type value_type;//节点值类型
    ...
    //rb_tree node traits //rb_tree 的节点 traits
    template<class T>
    struct rb_tree_node_traits
    {
        typedef rb_tree_color_type color_type; //颜色类型

        typedef rb_tree_value_traits<T>  value_traits; //节点值 traits
        typedef typename value_traits::key_type key_type; //键值类型
        typedef typename value_traits::mapped_type mapped_type; //map 的节点值类型
        typedef typename value_traits::value_type value_type; //节点值类型

        typedef rb_tree_node_base<T>* base_ptr; //指向节点基类的指针
        typedef rb_tree_node<T>* node_type; //指向节点的指针
    };
    ...
    struct rb_tree_node_base
    {
        ...
        node_ptr get_node_ptr()// 获取节点指针
        {
            return static_cast<node_ptr>(&*this);
        }

        node_ptr &get_node_ptr_ref() //获取节点指针的引用
        {
...
//        rb_tree_node(const T& value):value_(value){}
//        rb_tree_node(T&& value):value_(std::move(value)){}
//
//        template<class ...Args>
//        rb_tree_node(Args&& ...args):value_(std::forward<Args>(args)...){}


        base_ptr get_base_ptr() //获取节点指针
        {
    ...
    struct rb_tree_traits
    {
        ...
        typedef value_type *pointer;//指向节点值的指针
        typedef value_type &reference;//指向节点值的引用
        typedef const value_type *const_pointer;//指向节点值的常量指针
        typedef const value_type &const_reference;//指向节点值的常量引用

        typedef rb_tree_node_base<T> base_type;//节点基类
        typedef rb_tree_node<T> node_type;//节点类型

        typedef base_type* base_ptr;//指向节点基类的指针
        typedef node_type* node_ptr;//指向节点的指针
    };
    ...
    struct rb_tree_iterator_base:public mystl::iterator<mystl::bidirectional_iterator_tag,T>
    {
        ...
        void inc()
        {
            ...
            {
                node=rb_tree_min(node->right);//右子树不为空，下一个节点为右子树的最小节点
            }
            else
            {//右子树为空，向上寻找，直到找到一个节点，该节点为其父节点的左子节点
                auto y=node->parent;
                while(node==y->right)//如果当前节点为其父节点的右子节点，则继续向上寻找
                {
                    node=y;
                    y=y->parent;
                }
                if(node->right!=y)//如果当前节点不为其父节点的右子节点，则当前节点为下一个节点
                {
                    node=y;
                }
            }
...
// 参数一代表数据类型，参数二代表键值比较类型
    ...
    public:
        ...
        mystl::pair<iterator, iterator>
        equal_range_unique(const key_type& key) // 函数作用：返回一个 pair，其 first 成员是指向第一个键值为 key 的元素的迭代器，
                                                // second 成员是指向最后一个键值为 key 的元素的下一个位置的迭代器。
        {
