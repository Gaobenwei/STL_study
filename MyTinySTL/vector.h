 ...
 *     // 重新分配空间并在 pos 处插入元素
 ...
 796 此函数逻辑不懂
 */
#ifndef STL_STUDY_VECTOR_H
#define STL_STUDY_VECTOR_H
...
private:
    //helper functions
    //initialize /destory
    void try_init()noexcept; //默认空间

    void init_space(size_type size,size_type cap); //指定大小

    void fill_init(size_type n,const value_type & value); //用某个数值填充构建

    template<class Iter>
    void range_init(Iter first,Iter last);  //范围初始化
...
vector<T>& vector<T>::operator=(const vector& rhs)
{
    if(this!=&rhs)
    {
        ...
        {
            mystl::copy(rhs.begin(),rhs.begin()+size(),this->begin());
            mystl::uninitialized_copy(rhs.begin()+size(),rhs.end(),this->end_);
...
void vector<T>::reserve(size_type n)
{
    if(capacity()<n)
    {
        ...
        data_allocator ::deallocate(begin_,cap_-end_);
        begin_=tmp;
        end_=tmp+old_size;
        cap_=begin_+n;
    }
...
typename vector<T>::iterator vector<T>::emplace(const_iterator pos, Args &&...args) {
        ...
        if(end_!=cap_ && xpos==end_) //位置足够且在末尾加入
        {
            data_allocator ::construct(mystl::address_of(*end_),mystl::forward<Args>(args)...);
            ++end_;
        }
        else if(end_!=cap_) //位置足够但不是在末尾加入
        {
            //先将最后一个元素加入end_,在xpos--end_(不包括)移到 [,end_)中
            auto new_end=end_;
...
template <class T>void vector<T>::destory_and_recover(iterator first, iterator last, size_type n)
{
