...
#ifndef STL_STUDY_QUEUE_H
#define STL_STUDY_QUEUE_H
...
// 参数三代表比较权值的方式，缺省使用 mystl::less 作为比较方式
    ...
    public:
        typedef Container                           container_type;
        typedef Compare                             value_compare;  // 元素比较规则
        // 使用底层容器的型别
        typedef typename Container::value_type      value_type;
        typedef typename Container::size_type       size_type;
        typedef typename Container::reference       reference;
    ...
    private:
        container_type c_;  // 用底层容器表现 priority_queue
        value_compare  comp_;  // 比较器
    ...
    public:
        ...
        explicit priority_queue(size_type n)
                :c_(n)
        {
            mystl::make_heap(c_.begin(), c_.end(), comp_); // 建堆
        }
