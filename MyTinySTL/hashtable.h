//
// Created by 11048 on 2023/6/16.
//
// 这个头文件包含了一个模板类 hashtable
// hashtable : 哈希表，使用开链法处理冲突
#ifndef STL_STUDY_HASHTABLE_H
#define STL_STUDY_HASHTABLE_H
...
namespace mystl
{
    ...
    struct hashtable_node
    {
        ...
        hashtable_node(hashtable_node&& node)noexcept:next(node.next),value(mystl::move(node.value)){node.next= nullptr;}
    };

    //value traits
    template<class T,bool>  //bool 为 true 时，T 为 pair 类型
    struct ht_value_traits_imp
    {
        ...
        template<class Ty> //
        static const key_type& get_key(const Ty& value) //返回 value 的 key
        {
            return value;
        }

        template<class Ty>
        static const value_type & get_value(const Ty& value) //返回 value 的 value
        {
            return value;
        }
    };
    ...
    //模板参数的意义：T 为 value_type,HashFun 为哈希函数，KeyEqual 为键值比较函数，Alloc 为空间配置器
    ...
    template<class T>
    struct ht_local_iterator; //本地迭代器 与上述迭代器的区别在于，本地迭代器只能遍历一个桶中的元素
    ...
    {
        typedef mystl:: hashtable<T,HashFun,KeyEqual> hashtable;
        typedef ht_iterator_base<T,HashFun,KeyEqual> base;
        typedef mystl::ht_iterator<T,HashFun,KeyEqual> iterator;
        typedef mystl::ht_const_iterator<T,HashFun,KeyEqual> const_iterator;
        typedef hashtable_node<T>* node_ptr; //节点指针
        typedef hashtable* contain_ptr; //hashtable 指针
    ...
    struct ht_iterator:public ht_iterator_base<T,Hash,KeyEqual>
    {
        ...
        using base::node; //迭代器当前所指节点
        using base::ht; //指向迭代器所在容器

        ht_iterator()=default;
        ht_iterator(node_ptr n,contain_ptr t):base{n,t}{}
        ht_iterator(const iterator& rhs):base{rhs.node,rhs.ht}{}
        ht_iterator(const const_iterator& rhs):base{rhs.node,rhs.ht}{} //const_iterator 转换为 iterator
        ...
        iterator & operator=(const const_iterator& rhs) //返回值为 iterator,与ht_iterator 的区别:const_iterator 转换为 iterator
        {
            if(this!=&rhs)
            {
                node=rhs.node;
        ...
        iterator& operator++()
        {
            ...
            if(node== nullptr)
            {
                ...
                auto index = ht->hash(value_traits::get_key(old->value));  //求出下一个 bucket 的索引
                while(!node && ++index < ht->bucket_count_)  //如果下一个 bucket 为空，且索引小于 bucket 的数量
                    node = ht->buckets_[index];  //跳到下一个 bucket 的起始处
            }
            return *this;
        }
    ...
    struct ht_const_iterator:public ht_iterator_base<T,Hash,KeyEqual>
    {
        ...
        ht_const_iterator(node_ptr n,contain_ptr t):base{n,t}{} //构造函数
        ht_const_iterator(const iterator& rhs):base{rhs.node,rhs.ht}{}
        ht_const_iterator(const const_iterator& rhs):base{rhs.node,rhs.ht}{}
    ...
    struct ht_local_iterator:public mystl::iterator<mystl::forward_iterator_tag,T>
    {
        ...
        node_ptr node; //迭代器当前所指节点
        //size_type bucket; //迭代器当前所指节点所在的 bucket
    ...
    //local_iterator与ht_iterator之间的详细区别: local_iterator不需要记录bucket的位置，因为它只在一个bucket中进行遍历
...
// 参数一代表数据类型，参数二代表哈希函数，参数三代表键值相等的比较函数
    ...
    public:
        ...
        typedef mystl::vector<node_ptr> bucket_type; //桶
    ...
    private:
        //以下成员表现hashtaable
        bucket_type buckets_; //桶
        size_type bucket_count_; //桶的数量
        size_type size_; //元素的数量
        float mlf_;
        hasher hash_;
        key_equal equal_;
    ...
    private:
        ...
        bool is_eqqual(const key_type& key1,const key_type& key2) const
        {
            return equal_(key1,key2);
        }
        ...
        {//返回迭代器
            return const_iterator(node, const_cast<hashtable *>(this)); // const_cast<hashtable *>(this) 为了调用begin()函数
        }

        iterator M_begin()noexcept
        {
            for(size_type n=0;n<bucket_count_;++n)//遍历桶
            {
                if(buckets_[n]!=nullptr)//找到第一个不为空的桶
                {
                    return iterator(buckets_[n],this);
                }
            }
            return iterator(nullptr,this); //如果没有找到，返回空迭代器
        }

        const_iterator M_begin()const noexcept
        {
            for(size_type n=0;n<bucket_count_;++n)
            {
                if(buckets_[n])  //与 buckets_[n]!=nullptr 相同
                {
    ...
    public:
        ...
        hashtable(hashtable&& rhs) noexcept:bucket_count_(rhs.bucket_count_),size_(rhs.size_),mlf_(rhs.mlf_),hash_(rhs.hash_),equal_(rhs.equal_)
        {
        ...
        //修改容器相关操作
        //emplace emplace_hint insert insert_noresize
        template<class... Args>
                iterator emplace_multi(Args&&... args); //构造元素并插入

        template<class... Args>
                pair<iterator,bool> emplace_unique(Args&&... args); //构造元素并插入，保证元素唯一

        // [note]: hint 对于 hash_table 其实没有意义，因为即使提供了 hint，也要做一次 hash，
        // 来确保 hash_table 的性质，所以选择忽略它

        template<class... Args>
                iterator emplace_multi_use_hint(const_iterator hint, Args&&... args) //构造元素并插入
        {
            return emplace_multi(mystl::forward<Args>(args)...);
        }

        template<class... Args>
                iterator emplace_unique_use_hint(const_iterator hint, Args&&... args) //构造元素并插入，保证元素唯一
        {
        ...
        pair<iterator,bool> insert_unique(const value_type& value)//插入元素，保证元素唯一,返回值为迭代器和是否插入成功
        {
            rehash_if_need(1);
        ...
        //note:hint对于hashtable没有意义，因为即使提供了hint，也要做一次hash，来确保hashtable的性质，所以选择忽略它
        ...
        //bucket interface 作用：返回桶的数量，返回某个元素所在的桶，返回桶的元素数量
        ...
        local_iterator end(size_type n)noexcept //返回桶的尾部
        {
            MYSTL_DEBUG(n<size_);
            return nullptr;
        }
        ...
        size_type bucket_count()const noexcept { return bucket_count_; }  //返回桶的数量
        size_type max_bucket_count()const noexcept { return ht_prime_list[PRIME_NUM-1]; } //返回最大桶的数量

        size_type bucket_size(size_type n)const noexcept; //返回桶的元素数量
        size_type bucket(const key_type& key)const //返回某个元素所在的桶
        {
            return hash(key);
        }

        //hash policy : 返回hash函数，返回key_equal函数

        float load_factor() const noexcept  //返回负载因子
        { return bucket_count_ != 0 ? (float)size_ / bucket_count_ : 0.0f; }
        ...
        void rehash(size_type count); //重建表格，使得桶的数量至少为count，并且元素个数/桶的数量 <= max_load_factor()
    ...
    emplace_multi(Args&& ...args)
    {
        ...
        try
        {
            if ((float)(size_ + 1) > (float)bucket_count_ * max_load_factor())
                rehash(size_ + 1);
        }
        catch (...)
        {
    ...
    {
        const auto n = hash(value_traits::get_key(value)); // 计算hash值
        auto first = buckets_[n];
        for (auto cur = first; cur; cur = cur->next) // 检查链表中是否有相同的键值
        {
...
// 删除键值为 key 的节点
    ...
    rehash(size_type count)
    {
        ...
        else
        {
            if ((float)size_ / (float)n < max_load_factor() - 0.25f &&
                (float)n < (float)bucket_count_ * 0.75)  // worth rehash
            {
                replace_bucket(n);
            }
        }
    ...
    equal_range_multi(const key_type& key)
    {
        ...
        for (node_ptr first = buckets_[n]; first; first = first->next)
        {
            ...
            { // 如果出现相等的键值
                ...
                for (auto m = n + 1; m < bucket_count_; ++m)
    ...
    swap(hashtable& rhs) noexcept
    {
        if (this != &rhs)
        {
            ...
            mystl::swap(bucket_count_, rhs.bucket_count_);
...
// copy_init 函数
    ...
    {
        bucket_count_ = 0;
        buckets_.reserve(ht.bucket_count_);
        buckets_.assign(ht.bucket_count_, nullptr);
        try
        {
            for (size_type i = 0; i < ht.bucket_count_; ++i)
            {
            ...
            bucket_count_ = ht.bucket_count_;
            mlf_ = ht.mlf_;
            size_ = ht.size_;
        }
        catch (...)
...
// rehash_if_need 函数
    ...
    {
        if (static_cast<float>(size_ + n) > (float)bucket_count_ * max_load_factor())
            rehash(size_ + n);
    }
