...
//   * push_back
//   * insert
#ifndef STL_STUDY_LIST_H
#define STL_STUDY_LIST_H
...
namespace mystl
{
    template<class T>struct list_node_base; //节点指针基类,不包含数据
    ...
    //list迭代器设计
    template<class T>
    struct list_iterator:mystl::iterator<mystl::bidirectional_iterator_tag,T>
    {
...
// 删除 pos 处的元素
    ...
    typename list<T>::iterator list<T>::erase(const_iterator first,const_iterator last)
    {
        if(first!=last)
        {
            unlink_nodes(first.node_,last.node_);
            while (first!=last)
            {
    ...
    void list<T>::reverse()
    {
        ...
        {
            mystl::swap(i.node_->prev,i.node_next);
            i.node_=i.node_prev;
        }
...
// 在 pos 处插入 [first, last) 的元素
    ...
    // 对 list 进行归并排序，返回一个迭代器指向区间最小元素的位置 逻辑没有搞懂
    template <class T>
    template <class Compared>
    typename list<T>::iterator
