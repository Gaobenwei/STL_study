...
//   * emplace_hint
//   * insert
#ifndef STL_STUDY_UNORDERED_MAP_H
#define STL_STUDY_UNORDERED_MAP_H
...
namespace mystl
{
    ...
    public:
        ...
        // 修改容器相关
        // emplace / emplace_hint / insert / erase / clear
        ...
        pair<iterator,iterator> equal_range(const key_type& key) {return ht_.equal_range_unique(key);}  //返回一个pair，first指向第一个匹配元素，second指向最后一个匹配元素的下一个位置
        pair<const_iterator,const_iterator> equal_range(const key_type& key) const {return ht_.equal_range_unique(key);}

        // 桶接口
        local_iterator beign(size_type n) noexcept {return ht_.begin(n);}
    ...
    void swap(unordered_map<Key, T, Hash, KeyEqual>& lhs,
              unordered_map<Key, T, Hash, KeyEqual>& rhs) noexcept(noexcept(lhs.swap(rhs)))
    {
        lhs.swap(rhs);
    }
    ...
    public:
        ...
        unordered_multimap(unordered_multimap&& rhs) noexcept : ht_(mystl::move(rhs.ht_)) {}
        ...
        pair<iterator,iterator> equal_range(const key_type& key) // 返回一个迭代器对，表示特定键值的元素区间
...
#endif //STL_STUDY_UNORDERED_MAP_H
