...
#ifndef STL_STUDY_UNORDERED_SET_H
#define STL_STUDY_UNORDERED_SET_H
...
namespace mystl
{
    ...
    public:
        ...
        unordered_set(InputIterator first,InputIterator last,const size_type bucket_count=100,
        ...
        {
            ht_.clear();
            ht_reserve(ilist.size());
            for(auto first=ilist.beign(),last=ilist.end();first!=last;++first)
            {
                ht_.insert_unique_noresize(*first);
            }
            return *this;
        ...
        iterator beign() noexcept { return ht_.begin(); }
        ...
        {
            auto result=ht_.emplace_unique(mystl::forward<Args>(args)...);
        ...
        void swap(unordered_set& rhs) noexcept { ht_.swap(rhs.ht_); }
    ...
    void swap(unordered_set<Key, Hash, KeyEqual>& lhs,
              unordered_set<Key, Hash, KeyEqual>& rhs) noexcept(noexcept(lhs.swap(rhs)))
    {
        lhs.swap(rhs);
    }
...
#endif //STL_STUDY_UNORDERED_SET_H
